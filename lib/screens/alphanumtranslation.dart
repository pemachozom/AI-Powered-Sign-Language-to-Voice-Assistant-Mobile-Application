...
class AlphaNumTranslation extends StatefulWidget {
  final Map<String, dynamic> token;

  const AlphaNumTranslation({Key? key, required this.token}) : super(key: key);

  @override
  State<AlphaNumTranslation> createState() => _AlphaNumTranslationState();
}

class _AlphaNumTranslationState extends State<AlphaNumTranslation> {
    ...
    if (cameraController != null && cameraController!.value.isInitialized) {
      try {
        ...
        if (imageFile != null) {
          ...
          await speakPredictedClass(responseBody);
        }
      } catch (e) {
        print(e);
      }
  ...
  Future<http.Response> sendImageToApi(XFile imageFile) async {
    ...
    final apiUrl = 'http://10.9.90.205:5002/predict';
    // final apiUrl = 'http://192.168.1.48:5002/predict';
    // final apiUrl = 'http://192.168.47.115:5002/predict';
  ...
  Future<void> speakPredictedClass(String predictedClass) async {
